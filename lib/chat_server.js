...
function JoinRoom(socket) {
    socket.on('joinRoom', function (memID, r_index) {
        ...
        try {
            ...
            var getChatlistQuery = "SELECT r_index, roomName FROM roomName WHERE r_index = $1";
            const getChatlistResult = await client.query(getChatlistQuery, [r_index]);
            const chatlistRow = getChatlistResult.rows[0];
            ...
            await client.query(getChatContentQuery, [r_index]);


            for (var i in chatlistRow) {
                var obj = JSON.parse(chatlistRow[i].chatMsgList);
                for (var j = 0; j < obj.number; j++) {
                    var toJSON = JSON.stringify(obj.message[j].msg);
                    obj.message[j].msg = encryption.decodeMessage(toJSON);
                }
                chatlistRow[i].chatMsgList = JSON.stringify(obj);
            }
            var rows = result;
            for (var i in rows) {
