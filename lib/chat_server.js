...
async function CreateMember(socket) {
    socket.on('createMember', async function (data) {
        ...
        try {
            const result = await client.query(insertQuery, insertParam);
            const resultRows = result.rows;
 ...
 * output : success message
 */
async function AddFriendList(memID) {

    const selectMindexFriendList = "SELECT m_index FROM MemberInfo WHERE memID = ?;";
    const [rows, fields] = await db2.execute(selectMindexFriendList, [memID]);
    const m_index = 0;

    const insertNewFriendList = "INSERT INTO FriendList (m_index,memID) VALUES (?,?);";
    const aaa = await db2.execute("", [m_index, memID]);
...
function AddNewFriend(socket) {
    socket.on('addNewFriend2', async function (friendData) {
        ...
        try {
            var checkIdQuery = "SELECT m_index FROM MemberInfo WHERE memID = $1;";
            const checkIdResult = await client.query(checkIdQuery, [memID]);
            var queryResult = checkIdResult.rows[0];

            // 아이디가 등록되어 있으면 fList를 m_index를 사용해 FriendList 테이블에서 가져온다.
            // 아이디가 등록되어 있지 않으면 FriendList에 m_index를 사용해 등록한다.            
            var getFListQuery = "SELECT fList FROM FriendList WHERE m_index = $1;";
            const getFListResult = await client.query(getFListQuery, [queryResult[0].m_index]);
            const existFriendList = getFListResult.rows[0].fList;
            ...
            for (var i = 0; i < existFriendList.list.length; i++) {
                ...
                if (friendInfo.length == 0 || existFriendList.list[i] == friendInfo[0].m_index || friendInfo[0].m_index == out_m_index)
                    socket.emit('addNewFriend2', 'false');
            }

            existFriendList.list.push(friendInfo[0].m_index);
            friendCount += 1;
            existFriendList.number = friendCount;

            let newFriendList = existFriendList;


            var updateDBQuery = "UPDATE FriendList SET fList = $1 WHERE m_index = $2;";
            const zadfasResult = await client.query(updateDBQuery, [newFriendList, out_m_index]);
...
function GetChatRoom(socket) {
    socket.on('getChatRoom', async function (data, callback) {
        ...
        try {
            ...
            const memberChatList = JSON.parse(rawInfo);
            ...
            var chatListQuery = "SELECT r_index, roomName FROM ChatList WHERE r_index in (" + whereString + ")";
            const rawChatList = await client.query(chatListQuery);
            const chatRoomList = rawChatList.rows[0];
...
function MakeChatList(socket, row = []) {
    ...
    for (var i = 0; i < row.num; i++) {
        list += row.rIndexList[i];
        if (i != row.num - 1)
            list += ", ";
    }
