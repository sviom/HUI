...
function GetFriends(socket) {
    socket.on('checkFriends', async function (data, callback) {
        ...
        var sss = await client.query(query, [userId]);
        var number = sss.rows[0].fList.number;
        if (number > 0) {
            socket.emit('checkFriendsResult', { friendList: sss.rows[0].fList });
        } else {
            socket.emit('checkFriendsResult', { friendList: false });
        }
        // join 해야할걸 왜 두번 쿼리하고 있는지 
