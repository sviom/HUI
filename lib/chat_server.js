...
async function prevMessage(socket, r_index, j, obj) {
    const data = await getPicture(obj.message[j].sender);
    socket.emit('message', {
        sender: obj.message[j].sender,
        nick: obj.message[j].nick,
        text: obj.message[j].msg,
        r_index: r_index,
        pic: data,
        time: obj.message[j].time
    });
}
    ...
    socket.on('message', async function (message) {
        try {
            ...
            if (CHAT_LIST[message.room].length == 5) {
                ...
                for (var i in tmp) {
                    str += tmp[i] + ',';
                    tempSentence.message.push(tmp[i]);
                }
                tempSentence.number = tmp.length;
                ...
                await client.query(insertChatQuery, [sentence, message.room, sendedTime]);
            ...
            const photo = await getPicture(message.sender);
            socket.broadcast.to(message.room).emit('message', {
                sender: message.sender,
                nick: message.nick,
                text: message.text,
...
async function getPicture(memID) {
    try {
