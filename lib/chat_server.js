...
function verifyMember(socket) {
    ...
    socket.on('verifyMember', async function (data, callback) {
        ...
        var selectQuery =  'SELECT count(*) exist, m_index FROM `MemberInfo` WHERE memID = ? and memPW = ?';
        var selectParams =  [identification.memID, encryptedPw];
        const resss = await client.query(selectQuery, selectParams);

        const result = resss[0];
...
function CreateMember(socket) {
    socket.on('createMember', async function (data) {
        ...
        var selectQuery =  'SELECT memID FROM MemberInfo WHERE memID = ?';
